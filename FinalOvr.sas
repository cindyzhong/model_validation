...
proc partition data=public.tr samppctevt=100 eventprop=0.35
               event="1" seed=10 nthreads=1;
   by TARGET;
   ods output OVERFREQ=outFreq;
   output out=casuser.TrOvr freqname=_Freq2_;
run;

proc print data=casuser.TrOvr(obs=20);
run;

ods noproctitle;
libname _tmpcas_ cas caslib="CASUSER";
proc cardinality data=casuser.TrOvr outcard=_tmpcas_.varSummaryTemp 
...
proc partition data=casuser.TrOvr partind samppct=65 seed=1;
	by TARGET;
