...
proc genmod data=resp;
   class id treatment(ref="P") center(ref="1") sex(ref="M")
      baseline(ref="0");
   model outcome(event='1')=treatment center sex age baseline / dist=bin type3;
   repeated subject=id(center) / corr=unstr corrw;
run;

proc glimmix data=resp;
	class id treatment(ref="P") center(ref="1") sex(ref="M");
	model outcome(event='1')=treatment center sex age baseline / dist=binary link=logit solution;
	random _residual_ /type=un subject=id(center) g ;
	random _residual_;
	covtest 'Is there correlation?' GLM;
run;
