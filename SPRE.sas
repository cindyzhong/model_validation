/*-- Seasonal Model for the Airline Series --*/
  /* proc arima data=sashelp.air;
   ...
   run;/*-- Seasonal Model for the Airline Series --*/
...
proc logistic data=LOCALLIB.HMEQ_TRAIN plots=all;
	class REASON JOB / param=glm;
	model BAD(event='1')=REASON JOB LOAN MORTDUE VALUE YOJ DEROG DELINQ CLAGE NINQ 
		CLNO DEBTINC / link=logit ctable selection=backward slstay=0.05 
